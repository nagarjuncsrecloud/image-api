...
def create_test_image():
    """Generates a real in-memory JPEG image."""
    img = Image.new("RGB", (100, 100), color="red")
    img_bytes = io.BytesIO()  # Now io is defined
    img.save(img_bytes, format="JPEG")
    return img_bytes.getvalue()  # Return valid image bytes

@patch("app.main.requests.get")  # Make sure this path matches your imports
def test_load_image_from_url(mock_get):
    print("Mocking requests.get")  # Debugging

    # Mock Response Object with Real Image
    mock_response = mock_get.return_value
    mock_response.status_code = 200
    mock_response.content = create_test_image()  # Use actual image bytes
    mock_response.raw = BytesIO(mock_response.content)  # Simulating raw stream

    response = client.get("/load-from-url/?image_url=https://dummyimage.com/600x400/000/fff")

    print("Mock called:", mock_get.called)  # Should be True
